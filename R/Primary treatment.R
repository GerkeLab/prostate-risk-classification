...
################################################################################################ NCDB ################ 
# All patients are prostate cancer as primary site so no need to filter for that
# But patients don't always receive radiation or surgery for prostate first
# Need to filter for that
# Had to have multiple conditions for each because some recordings started in 2003, 2010
...
treatment_ncdb <- ncdb %>% 
  ...
  mutate(rad_site = case_when(
    # rad site for the first course treatment # will probably not use as is, could be removed if not
    RAD_TREAT_VOL %in% c(1:98) & 
      RAD_TREAT_VOL != 41 ~ "other_site",
    RAD_TREAT_VOL == 41 ~ "prostate",
    TRUE ~ NA_character_
  )) %>% 
  mutate(systemic_treatment = case_when( # Will probably not use as is, could be removed if not
    ...
    # So need to compare what 1st course happened first
      ...
      ((dx_of_first_chemo_only &
        ...
        dx_of_first_surg_prostate_only | 
           dx_of_first_rad_prostate_only) |
        (dx_of_first_surg_prostate_only == 
           dx_of_first_rad_prostate_only) ~ "same_time", # I can separate them if needed, let me know
      TRUE ~ NA_character_
  )) %>% 
  ...
  mutate(died_prior_systemic__planned_recommended = case_when(
    RX_SUMM_CHEMO == 85 &
      ...
      RX_SUMM_TRNSPLNT_ENDO == 85 ~ "death_prior_systemic", # Can separate them if needed, let me know
    TRUE ~ NA_character_
    )) %>% 
  mutate(palliative_care = case_when( 
    ...
    # This include palliative care after treatment
    # See next var for palliative_care_only
