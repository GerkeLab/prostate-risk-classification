...
prostate cancer data for SEER. During followup, `r nrow(risk_seer[risk_seer$os==1,])` (`r round(nrow(risk_seer[risk_seer$os==1,])/nrow(risk_seer)*100, 2)`%) men died.

*NCDB*

In NCDB, there were `r length(unique(risk_ncdb$PUF_CASE_ID))` particiapnts with prostate cancer, of which `r nrow(risk_ncdb[risk_ncdb$os==1,])` (`r round(nrow(risk_ncdb[risk_ncdb$os==1,])/nrow(risk_ncdb)*100, 2)` %) died
...
base_table <- print(table_one_seer)
colnames_seer <- colnames(base_table)
# table_one_seer_race <- CreateTableOne(vars = seer_vars, strata = c("RACE1V"), 
# data = risk_seer)
...
# table_one_ncdb_race <- CreateTableOne(vars = ncdb_vars, strata = c("RACE"), data = risk_ncdb)
base_table <- cbind(base_table, print(table_one_ncdb)[,1])
base_table
```
