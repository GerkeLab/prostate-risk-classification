...
Analyses were performed overall and stratified by self-reported race.
...
`r min(unlist(seer_c_index$concord))` to `r max(unlist(seer_c_index$concord))`. 
...
`r min(unlist(ncdb_c_index$concord)))`, while the highest at 
`r max(unlist(ncdb_c_index$concord)))` was observed in GCP. 
